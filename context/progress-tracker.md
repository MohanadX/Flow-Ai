...
- AI sidebar transition & mount fixes:
  ...
  - Added a `Retry` button using `RefreshCw` icon in the AI sidebar's chat error state that remounts the `ChatMessages` component to attempt re-fetching if `useFeedMessages` fails.
...
- 30-canvas-optimize:
  ...
  - Reduced `LiveblocksProvider` `throttle` from 80ms to 32ms (~30fps) and removed the redundant explicit `lostConnectionTimeout={5000}` (matches SDK default).
