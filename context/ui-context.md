...
All colors are defined as CSS custom properties in globals.css and mapped to Tailwind tokens via @theme inline. Components must use these tokens — no hardcoded hex values or raw Tailwind color classes like zinc-800 .
...
Lucide React. Stroke-based icons only. Sizes: h-4 w-4 for inline, h-5 w-5 for buttons h-8 w-8 for feature icons in empty states.
